//! `noirdoc`: turn `.nr` source into mdBook-ready Markdown.
//!
...
//!
//! Output layout, given `noirdoc std -o docs`:
//! ```text
//! docs/
//!   book.toml
//!   src/
...
use crate::DocArgs;
use noir_common::ast::{TopLevel, TopLevelNode, Type};
use noir_front::parse;
...
pub fn generate(args: &DocArgs) -> Result<(), ()> {
    let files = collect_nr_files(&args.inputs);
    if files.is_empty() {
        eprintln!("noirdoc: no .nr files found in {:?}", args.inputs);
        return Err(());
    }

    let src_dir = args.out.join("src");
    if let Err(e) = std::fs::create_dir_all(&src_dir) {
        eprintln!("noirdoc: cannot create {}: {}", src_dir.display(), e);
        return Err(());
    }
    ...
    for (strip_base, file) in &files {
        ...
        let markdown = match render_file(&title, file) {
            Ok(md) => md,
            Err(()) => {
                eprintln!("noirdoc: skipping {} (failed to parse)", file.display());
                continue;
            }
        };
        ...
        if let Some(parent) = page_path.parent() {
            if let Err(e) = std::fs::create_dir_all(parent) {
                eprintln!("noirdoc: cannot create {}: {}", parent.display(), e);
                continue;
            }
        }
        if let Err(e) = std::fs::write(&page_path, markdown) {
            eprintln!("noirdoc: cannot write {}: {}", page_path.display(), e);
            continue;
        }
    ...
    if pages.is_empty() {
        eprintln!("noirdoc: no pages generated");
        return Err(());
    }
    ...
    write_book_toml(&args.out, "noir std")?;
    write_theme(&args.out)?;

    println!(
        "noirdoc: wrote {} page(s) to {}",
        pages.len(),
        args.out.display()
    );
    Ok(())
...
/// Recursively gather `.nr` files from each input. Returns `(strip_base, file)`
...
/// file's parent, so a lone `foo/bar.nr` titles as just `bar`.
fn collect_nr_files(inputs: &[PathBuf]) -> Vec<(PathBuf, PathBuf)> {
    let mut out = Vec::new();
    for input in inputs {
        if input.is_dir() {
            ...
            eprintln!("noirdoc: skipping {} (not a .nr file)", input.display());
        }
    }
    out
}
...
fn walk_dir(base: &Path, dir: &Path, out: &mut Vec<(PathBuf, PathBuf)>) {
    let entries = match std::fs::read_dir(dir) {
        Ok(e) => e,
        Err(e) => {
            eprintln!("noirdoc: cannot read {}: {}", dir.display(), e);
            return;
        }
    };
...
fn is_nr(p: &Path) -> bool {
    p.extension().and_then(|e| e.to_str()) == Some("nr")
}

/// Strip `strip_base` off `file` and drop the extension to form a module title:
/// `std/dsp/osc.nr` with base `` -> `std/dsp/osc`; `foo/bar.nr` with base `foo`
...
fn render_file(title: &str, file: &Path) -> Result<String, ()> {
    let src = std::fs::read_to_string(file).map_err(|e| {
        eprintln!("noirdoc: cannot read {}: {}", file.display(), e);
    })?;
...
fn attr_prefix(attributes: &[noir_common::ast::Attribute]) -> String {
...
fn fmt_generics(generics: &[noir_common::ast::GenericParam]) -> String {
...
fn write_summary(src_dir: &Path, pages: &[(String, PathBuf)]) -> Result<(), ()> {
    ...
    std::fs::write(&path, summary).map_err(|e| {
        eprintln!("noirdoc: cannot write {}: {}", path.display(), e);
    })
}
...
/// mdBook theme, emitted so a plain `noirdoc` run needs no manual theming.
///
...
/// embedded so a plain `noirdoc` run ships self-contained fonts. `(filename,
...
fn write_theme(out: &Path) -> Result<(), ()> {
    ...
    if let Err(e) = std::fs::create_dir_all(&css_dir) {
        eprintln!("noirdoc: cannot create {}: {}", css_dir.display(), e);
        return Err(());
    }
    ...
    if let Err(e) = std::fs::create_dir_all(&fonts_dir) {
        eprintln!("noirdoc: cannot create {}: {}", fonts_dir.display(), e);
        return Err(());
    }
...
/// Write `bytes` to `path`, reporting any error under the `noirdoc:` prefix.
